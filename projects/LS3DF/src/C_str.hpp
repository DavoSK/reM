...
// which may be filled further by array-access functions.
   C_str &Assign(const char *cp, uint32_t size){
      C_rep *new_rep;
...
// Get const character reference inside of string on particular position.
   inline const char &operator [](uint32_t pos) const{ assert(pos<(Size()+1)); if(!rep) return *(char*)&rep; assert(!IsWide()); return rep->GetData()[pos]; }
   inline const char &operator [](int pos) const{ return operator[](uint32_t(pos)); }
   inline const wchar &GetW(uint32_t pos) const{ assert(pos<(Size()+1)); if(!rep) return *(wchar*)&rep; assert(IsWide()); return rep->GetDataW()[pos]; }
...
// If contents of string is shared among multiple strings, it is made unique.
   char &operator [](uint32_t pos){ assert(pos<(Size()+1)); if(!rep) return *(char*)&rep; assert(!IsWide()); MakeUniqueCopy(); return rep->GetData()[pos]; }
   char &operator [](int pos){ return operator[](uint32_t(pos)); }
   wchar &GetW(uint32_t pos){ assert(pos<(Size()+1)); if(!rep) return *(wchar*)&rep; assert(IsWide()); MakeUniqueCopy(); return rep->GetDataW()[pos]; }
...
// Get size of string data, without terminating NULL character.
   inline uint32_t Size() const{ if(!rep) return 0; size_t sz = rep->Size(); if(IsWide()) sz /= 2; return (uint32_t)sz; }
...
// Convert string to wide.
   C_str ToWide(){ if(!Size() || IsWide()) return *this; const char *cp = *this;
      ...
      for(uint32_t i=0; i<sz; i++) buf[i] = cp[i];
...
class C_xstr{
   ...
   typedef uint8_t _t;           //variable type using to hold the type

   enum{ TMP_STORAGE = (sizeof(int)+sizeof(_t)) * 6};
   uint8_t stor[TMP_STORAGE];    //local space used without allocating by new (up to N int-sized params)
                              //pointer to current storage, and next position (used during feeding params)
   uint8_t *curr_stor;
   uint32_t buf_pos;
   mutable uint32_t alloc_size;  //currently new-allocated buffer (pointed to by curr_stor), if 0, no alloc made
...
// Virtual allocation, directing alloc/free to same module.
   virtual uint8_t *Alloc(size_t sz) const{ return new uint8_t[sz]; }
   virtual void Free(uint8_t *vp) const{ delete[] vp; }

//----------------------------
// Get size of data of particular type.
   uint32_t GetDataSize(_t t, const void *dt) const{
      switch(t){
      case t_char: return sizeof(char);
      case t_int: return sizeof(int);
      case t_uint32_t: return sizeof(unsigned int);
...
// Add data of given type into feed buffer (manage allocation).
   void AddData(_t type, const void *data, uint32_t data_size = 0){
      if(built)
         throw std::exception("C_xstr already built, can't feed more parameters");
      assert(!built);
      if(!data_size)
         data_size = GetDataSize(type, data);
      uint32_t sz = sizeof(_t) + data_size;
      uint32_t buf_sz = alloc_size ? alloc_size : TMP_STORAGE;
      if(buf_pos + sz > buf_sz){
         ...
         if(!alloc_size){
            ...
            uint8_t *ns = Alloc(alloc_size);
            memcpy(ns, curr_stor, buf_pos);
            Free(curr_stor);
            curr_stor = ns;
         }
      }
      uint8_t *dt = curr_stor + buf_pos;
      *(_t*)dt = type;
...
// Get data from the feed buffer, advance input pointers to next data.
   const uint8_t *GetData(const uint8_t *&cs, uint32_t &cp) const{
      uint32_t buf_sz = (cs==stor) ? TMP_STORAGE : buf_pos;
...
// Build string from given format string and fed parameters (if not built yet).
   void Build() const{
      ...
      uint32_t buf_size = (str.Size() + 100) & -4;
      char *buf = new char[buf_size];
      uint32_t buf_pos = 0;
                              //iterators for getting fed params
      const uint8_t *cs = stor;
      uint32_t cp = 0;
            ...
            if(*c=='%' || *c=='#'){
               if(*p==*c){
                  ...
                  *fp++ = '%';
                  ...
                  const uint8_t *dt = GetData(cs, cp);
                  ...
                  *fp++ = tc[type]; *fp++ = '%'; *fp++ = 'n'; *fp = 0;
                  ...
                  case t_uint32_t: sprintf(tmp, fs, *(unsigned int*)dt, &add_size); break;
...
// Parameter feeding for basic types, using operator %.
   ...
   inline C_xstr &operator %(uint32_t d){ AddData(t_uint32_t, &d); return *this; }
   //inline C_xstr &operator %(unsigned int d){ operator %(uint32_t(d)); return *this; }
